...
const Home: React.FC = () => {
  ...
  const handlePageInputSubmit = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      ...
      if (!isNaN(parsed) && parsed >= 1 && parsed <= (data?.meta.totalPages ?? 1)) {
        setPage(parsed);
        setPageInput('');
      ...
      {!isLoading && data && (
        <>
            ...
            >
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                ...
                <TextField
                  ...
                  placeholder={`1–${data.meta.totalPages}`}
                  sx={{ width: 80 }}
                  inputProps={{ min: 1, max: data.meta.totalPages }}
                />
              </Box>
            </Box>
          )}
          ...
          <Typography variant="body2" color="text.secondary" textAlign="center" sx={{ mt: 2 }}>
            Showing {(page - 1) * PLAYERS_PER_PAGE + 1} - {Math.min(page * PLAYERS_PER_PAGE, data.meta.totalCount)} of{' '}
            {data.meta.totalCount} players
          </Typography>
        </>
      )}
    </Container>
